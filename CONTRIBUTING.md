...
Setup guides, [FAQ](/spacearr/faq), the more information we have on the [wiki](https://wiki.servarr.com/spacearr) the better.

# Development

Spacearr is written in C# (backend) and JS (frontend). The backend is built on the .NET6 (and _soon_ .NET8) framework, while the frontend utilizes Reactjs.
...
> Ensure startup project is set to `Spacearr.Console` and framework to `net6.0`
{.is-info}

1. First `Build` the solution in Visual Studio, this will ensure all projects are correctly built and dependencies restored
1. Next `Debug/Run` the project in Visual Studio to start Spacearr
...
```shell
dotnet clean src/Spacearr.sln -c Debug
```

1. Restore and Build debug configuration for the correct platform (Posix or Windows)

```shell
dotnet msbuild -restore src/Spacearr.sln -p:Configuration=Debug -p:Platform=Posix -t:PublishAllRids
```
...
- If you're adding a new, already requested feature, please comment on [GitHub Issues](https://github.com/Spacearr/Spacearr/issues) so work is not duplicated (If you want to add something not already on there, please talk to us first)
- Rebase from Spacearr's develop branch, do not merge
...
Spacearr utilizes nunit for its unit, integration, and automation test suite.
...
Spacearr uses a self hosted open access [Weblate](https://translate.servarr.com) instance to manage its json translation files. These files are stored in the repo at `src/NzbDrone.Core/Localization`

## Contributing to an Existing Translation

Weblate handles synchronization and translation of strings for all languages other than English. Editing of translated strings and translating existing strings for supported languages should be performed there for the Spacearr project.

The English translation, `en.json`, serves as the source for all other translations and is managed on GitHub repo.

## Adding a Language

Adding translations to Spacearr requires two steps

- Adding the Language to weblate
- Adding the Language to Spacearr codebase
