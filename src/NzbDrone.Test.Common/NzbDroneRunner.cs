...
namespace NzbDrone.Test.Common
{
    public class NzbDroneRunner
    {
        ...
        public void Start(bool enableAuth = false)
        {
            ...
            if (OsInfo.IsWindows)
            {
                consoleExe = "Spacearr.Console.exe";
            }
            else
            {
                consoleExe = "Spacearr";
            ...
            while (true)
            {
                ...
                {
                    TestContext.Progress.WriteLine($"Spacearr {Port} is started. Running Tests");
                    return;
                }

                TestContext.Progress.WriteLine("Waiting for Spacearr to start. Response Status : {0}  [{1}] {2}", statusCall.ResponseStatus, statusCall.StatusDescription, statusCall.ErrorException.Message);
        ...
        public void Kill()
        {
            try
            {
                if (_nzbDroneProcess != null)
                {
                    ...
                    if (_nzbDroneProcess.HasExited)
                    {
                        var log = File.ReadAllLines(Path.Combine(AppData, "logs", "Spacearr.trace.txt"));
        ...
        public void KillAll()
        {
            try
            {
                ...
                _processProvider.KillAll(ProcessProvider.SPACEARR_CONSOLE_PROCESS_NAME);
                _processProvider.KillAll(ProcessProvider.SPACEARR_PROCESS_NAME);
            }
            catch (InvalidOperationException)
            {
                // May happen if the process closes while being closed
        ...
        private void Start(string outputSpacearrConsoleExe)
        {
            StringDictionary envVars = new ();
            if (PostgresOptions?.Host != null)
            {
                envVars.Add("Spacearr__Postgres__Host", PostgresOptions.Host);
                envVars.Add("Spacearr__Postgres__Port", PostgresOptions.Port.ToString());
                envVars.Add("Spacearr__Postgres__User", PostgresOptions.User);
                envVars.Add("Spacearr__Postgres__Password", PostgresOptions.Password);
                envVars.Add("Spacearr__Postgres__MainDb", PostgresOptions.MainDb);
                envVars.Add("Spacearr__Postgres__LogDb", PostgresOptions.LogDb);

                TestContext.Progress.WriteLine("Using env vars:\n{0}", envVars.ToJson());
            }

            TestContext.Progress.WriteLine("Starting instance from {0} on port {1}", outputSpacearrConsoleExe, Port);

            var args = "-nobrowser -nosingleinstancecheck -data=\"" + AppData + "\"";
            _nzbDroneProcess = _processProvider.Start(outputSpacearrConsoleExe, args, envVars, OnOutputDataReceived, OnOutputDataReceived);
        }
