...
namespace NzbDrone.Common.Instrumentation
{
    public static class NzbDroneLogger
    {
        ...
        private static void RegisterConsole()
        {
            ...
            var logFormat = Enum.TryParse<ConsoleLogFormat>(Environment.GetEnvironmentVariable("SPACEARR__LOG__CONSOLEFORMAT"), out var formatEnumValue)
                ? formatEnumValue
        ...
        {
            RegisterAppFile(appFolderInfo, "appFileInfo", "spacearr.txt", 50, LogLevel.Info);
            RegisterAppFile(appFolderInfo, "appFileDebug", "spacearr.debug.txt", 500, LogLevel.Off);
            RegisterAppFile(appFolderInfo, "appFileTrace", "spacearr.trace.txt", 500, LogLevel.Off);
        }
        ...
        private static void RegisterGlobalFilters()
        {
            LogManager.Setup().LoadConfiguration(c =>
            {
                ...
                c.ForLogger("Spacearr.Http.Authentication.ApiKeyAuthenticationHandler").WriteToNil(LogLevel.Info);
            });
        }
