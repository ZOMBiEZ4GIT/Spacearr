...
namespace NzbDrone.Common.EnvironmentInfo
{
    public class RuntimeInfo : IRuntimeInfo
    {
        ...
        public RuntimeInfo(Logger logger, IHostLifetime hostLifetime = null)
        {
            ...
            // net6.0 will return Spacearr.dll for entry assembly, we need the actual
            // executable name (Spacearr on linux).  On mono this will return the location of
            ...
            if (entry != null)
            {
                ...
                IsWindowsTray = OsInfo.IsWindows && entry.ModuleName == $"{ProcessProvider.SPACEARR_PROCESS_NAME}.exe";
            }
        }
        ...
        public bool IsTray
        {
            get
            {
                if (OsInfo.IsWindows)
                {
                    return IsUserInteractive && Process.GetCurrentProcess().ProcessName.Equals(ProcessProvider.SPACEARR_PROCESS_NAME, StringComparison.InvariantCultureIgnoreCase);
                }
