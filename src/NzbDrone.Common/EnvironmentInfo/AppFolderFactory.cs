...
namespace NzbDrone.Common.EnvironmentInfo
{
    ...
    public class AppFolderFactory : IAppFolderFactory
    {
        ...
        public void Register()
        {
            ...
            {
                throw new SpacearrStartupException("Cannot create AppFolder, Access to the path {0} is denied", _appFolderInfo.AppDataFolder);
            }
            ...
            {
                throw new SpacearrStartupException("AppFolder {0} is not writable", _appFolderInfo.AppDataFolder);
            }
        ...
        private void MigrateAppDataFolder()
        {
            try
            {
                if (OsInfo.IsOsx)
                {
                    var userAppDataFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile, Environment.SpecialFolderOption.DoNotVerify), ".config", "Spacearr");
                ...
                // Exit if a spacearr.db already exists
                if (_diskProvider.FileExists(_appFolderInfo.GetDatabase()))
                {
                    return;
                }
            ...
            catch (Exception ex)
            {
                ...
                throw new SpacearrStartupException(ex, "Unable to migrate DB from nzbdrone.db to {0}. Migrate manually", _appFolderInfo.GetDatabase());
            }
        }
        ...
        private void RemovePidFile()
        {
            ...
            {
                _diskProvider.DeleteFile(Path.Combine(_appFolderInfo.AppDataFolder, "spacearr.pid"));
            }
        }
    }
}
