...
namespace NzbDrone.Host
{
    public static class Bootstrap
    {
        ...
        public static void Start(string[] args, Action<IHostBuilder> trayCallback = null)
        {
            try
            {
                Logger.Info("Starting Spacearr - {0} - Version {1}",
                ...
                switch (appMode)
                {
                    ...
                    default:
                    {
                        new HostBuilder()
                            ...
                            {
                                services.Configure<PostgresOptions>(config.GetSection("Spacearr:Postgres"));
                                services.Configure<AppOptions>(config.GetSection("Spacearr:App"));
                                services.Configure<AuthOptions>(config.GetSection("Spacearr:Auth"));
                                services.Configure<ServerOptions>(config.GetSection("Spacearr:Server"));
                                services.Configure<LogOptions>(config.GetSection("Spacearr:Log"));
                                services.Configure<UpdateOptions>(config.GetSection("Spacearr:Update"));
                            }).Build();
            ...
            {
                throw new SpacearrStartupException(ex);
            }
        ...
        public static IHostBuilder CreateConsoleHostBuilder(string[] args, StartupContext context)
        {
            ...
            var bindAddress = config.GetValue<string>($"Spacearr:Server:{nameof(ServerOptions.BindAddress)}") ?? config.GetValue(nameof(ConfigFileProvider.BindAddress), "*");
            var port = config.GetValue<int?>($"Spacearr:Server:{nameof(ServerOptions.Port)}") ?? config.GetValue(nameof(ConfigFileProvider.Port), 8787);
            var sslPort = config.GetValue<int?>($"Spacearr:Server:{nameof(ServerOptions.SslPort)}") ?? config.GetValue(nameof(ConfigFileProvider.SslPort), 8787);
            var enableSsl = config.GetValue<bool?>($"Spacearr:Server:{nameof(ServerOptions.EnableSsl)}") ?? config.GetValue(nameof(ConfigFileProvider.EnableSsl), false);
            var sslCertPath = config.GetValue<string>($"Spacearr:Server:{nameof(ServerOptions.SslCertPath)}") ?? config.GetValue<string>(nameof(ConfigFileProvider.SslCertPath));
            var sslCertPassword = config.GetValue<string>($"Spacearr:Server:{nameof(ServerOptions.SslCertPassword)}") ?? config.GetValue<string>(nameof(ConfigFileProvider.SslCertPassword));
            var logDbEnabled = config.GetValue<bool?>($"Spacearr:Log:{nameof(LogOptions.DbEnabled)}") ?? config.GetValue(nameof(ConfigFileProvider.LogDbEnabled), true);
        ...
        private static IConfiguration GetConfiguration(StartupContext context)
        {
            ...
            catch (InvalidDataException ex)
            {
                ...
                throw new InvalidConfigFileException($"{configPath} is corrupt or invalid. Please delete the config file and Spacearr will recreate it.", ex);
            }
        }
