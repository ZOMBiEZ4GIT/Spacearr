...
namespace NzbDrone.Update.UpdateEngine
{
    ...
    public class InstallUpdateService : IInstallUpdateService
    {
        ...
        public void Start(string installationFolder, int processId)
        {
            ...
            _logger.Info("Updating Spacearr from version {0} to version {1}", _detectExistingVersion.GetExistingVersion(installationFolder), BuildInfo.Version);

            Verify(installationFolder, processId);

            if (installationFolder.EndsWith(@"\bin\Spacearr") || installationFolder.EndsWith(@"/bin/Spacearr"))
            {
            ...
            _processProvider.FindProcessByName(ProcessProvider.SPACEARR_CONSOLE_PROCESS_NAME);
            _processProvider.FindProcessByName(ProcessProvider.SPACEARR_PROCESS_NAME);
            ...
            try
            {
                ...
                if (OsInfo.IsWindows)
                {
                    if (_processProvider.Exists(ProcessProvider.SPACEARR_CONSOLE_PROCESS_NAME) || _processProvider.Exists(ProcessProvider.SPACEARR_PROCESS_NAME))
                    {
                        _logger.Error("Spacearr was restarted prematurely by external process.");
                        return;
                    }
                }
                ...
                try
                {
                    ...
                    {
                        _diskProvider.SetFilePermissions(Path.Combine(installationFolder, "Spacearr"), "755", null);
            ...
            finally
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < 10; i++)
                    {
                        ...
                        if (_processProvider.Exists(ProcessProvider.SPACEARR_PROCESS_NAME))
                        {
                            _logger.Info("Spacearr was restarted by external process.");
                            break;
                        }
                    }
