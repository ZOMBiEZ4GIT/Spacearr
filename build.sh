...
UpdateVersionNumber()
{
    if [ "$SPACEARRVERSION" != "" ]; then
        echo "Updating Version Info"
        sed -i'' -e "s/<AssemblyVersion>[0-9.*]\+<\/AssemblyVersion>/<AssemblyVersion>$SPACEARRVERSION<\/AssemblyVersion>/g" src/Directory.Build.props
        sed -i'' -e "s/<AssemblyConfiguration>[\$()A-Za-z-]\+<\/AssemblyConfiguration>/<AssemblyConfiguration>${BUILD_SOURCEBRANCHNAME}<\/AssemblyConfiguration>/g" src/Directory.Build.props
        sed -i'' -e "s/<string>10.0.0.0<\/string>/<string>$SPACEARRVERSION<\/string>/g" distribution/osx/Spacearr.app/Contents/Info.plist
    fi
}
...
PackageFiles()
{
    ...
    cp -r $outputFolder/Spacearr.Update/$framework/$runtime/publish $folder/Spacearr.Update
...
PackageLinux()
{
    ...
    local folder=$artifactsFolder/$runtime/$framework/Spacearr
    ...
    echo "Removing Spacearr.Windows"
    rm $folder/Spacearr.Windows.*

    echo "Adding Spacearr.Mono to UpdatePackage"
    cp $folder/Spacearr.Mono.* $folder/Spacearr.Update
    if [ "$framework" = "net8.0" ]; then
        cp $folder/Mono.Posix.NETStandard.* $folder/Spacearr.Update
        cp $folder/libMonoPosixHelper.* $folder/Spacearr.Update
    fi
...
PackageMacOSApp()
{
    ...
    rm -rf $folder
    mkdir -p $folder
    cp -r distribution/osx/Spacearr.app $folder
    mkdir -p $folder/Spacearr.app/Contents/MacOS

    echo "Copying Binaries"
    cp -r $artifactsFolder/$runtime/$framework/Spacearr/* $folder/Spacearr.app/Contents/MacOS

    echo "Removing Update Folder"
    rm -r $folder/Spacearr.app/Contents/MacOS/Spacearr.Update
...
PackageWindows()
{
    ...
    echo "Removing Spacearr.Mono"
    rm -f $folder/Spacearr.Mono.*
    rm -f $folder/Mono.Posix.NETStandard.*
    rm -f $folder/libMonoPosixHelper.*

    echo "Adding Spacearr.Windows to UpdatePackage"
    cp $folder/Spacearr.Windows.* $folder/Spacearr.Update
...
BuildInstaller()
{
    ...
    ./_inno/ISCC.exe distribution/windows/setup/spacearr.iss "//DFramework=$framework" "//DRuntime=$runtime"
}
