...
`Program.cs`, after `app.UseAuthorization();` and the endpoint maps:
...
export const useChangePassword = () => useMutation({ mutationFn: (newPassword: string) => api.post('/api/v1/auth/password', { newPassword }) });
```
...
export default function AccountPage() {
  ...
  const submit = async (e: FormEvent) => { e.preventDefault(); await change.mutateAsync(pw); setPw(''); setDone(true); };
  return (
    <div className={s.page}>
      <h1>Account</h1>
      <section className={`card ${s.grid}`}>
      ...
      <form className={`card ${s.grid}`} onSubmit={submit}>
        ...
        <div className={s.row}><button className="btn btn-primary" disabled={change.isPending}>Change password</button>{done && <span className={s.ok}>Changed</span>}</div>
      </form>
