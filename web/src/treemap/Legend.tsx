...
export default function Legend({ colorBy, heatMode, categories = [] }: { colorBy: string; heatMode: string; categories?: string[] }) {
  ...
  if (colorBy === 'duplicates') return <div className={s.legend}><span><i className={s.swatch} style={{ background: 'var(--danger)' }} />Has a duplicate</span><span><i className={s.swatch} style={{ background: 'var(--muted)' }} />Unique</span></div>;
